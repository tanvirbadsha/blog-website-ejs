...
const Post = mongoose.model("Post", postsScema);
...
const post2 = new Post({
  ...
  content: "this is for testing. a static paragraph to remove the pain of removign again and again."
});
...
app.get('/', (req, res)=>{
  Post.find({}).then((foundPosts)=>{
    if(foundPosts.length === 0){
      Post.insertMany(defaultPosts).then(()=>console.log("Sucess")).catch((err)=> console.log(err));
      res.redirect('/');
    }else{
      res.render('home.ejs', {newContent:foundPosts});
    }
...
app.post('/compose', (req,res)=>{
    ...
    const content = _.upperFirst(req.body.blog);
...
app.get('/posts/:parameter', (req,res)=>{
  const typedParam =_.upperFirst(req.params.parameter);
  const decodedTitle = decodeURIComponent(typedParam);
  const formattedTitle = decodedTitle.replace(/-/g, ' ');
  console.log(typedParam);
  Post.findOne({title: formattedTitle}).then((foundPost)=>{
    if(foundPost){
      res.render('post.ejs', {title: foundPost.title, content: foundPost.content});
    }else{
      console.log("posts not found la");
    }
